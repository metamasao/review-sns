...
class CustomUserTest(TestCase):
    ...
    def test_custom_user_attribute(self):
        ...
        self.assertURLEqual(self.user1.get_absolute_url(), f'/accounts/{self.user1.username}/{self.user1.pk}/')
    ...
    def test_action(self):
        ...
        self.assertEqual(self.user1.actions.all()[0].action, action_content)
        self.assertEqual(self.user2.actions.all().count(), 0)
        self.assertURLEqual(self.user1.actions.all()[0].action_url, self.user2.get_absolute_url())
        self.assertEqual(self.user1.actions.all()[0].__str__(), action_content)
